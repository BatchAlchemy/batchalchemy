...
:: Try (in order): RIDER_JDK, rider64.exe.jdk, ..\jbr, JDK_HOME, JAVA_HOME.
:: ---------------------------------------------------------------------
SET JRE=

IF NOT "%RIDER_JDK%" == "" (
  IF EXIST "%RIDER_JDK%" SET "JRE=%RIDER_JDK%"
)

SET _JRE_CANDIDATE=
IF "%JRE%" == "" IF EXIST "%APPDATA%\JetBrains\Rider2023.2\rider64.exe.jdk" (
  SET /P _JRE_CANDIDATE=<"%APPDATA%\JetBrains\Rider2023.2\rider64.exe.jdk"
)
IF "%JRE%" == "" (
...
IF NOT EXIST "%JAVA_EXE%" (
  ECHO ERROR: cannot start JetBrains Rider.
  ECHO No JRE found. Please make sure RIDER_JDK, JDK_HOME, or JAVA_HOME point to a valid JRE installation.
  EXIT /B
)
...
:: ---------------------------------------------------------------------
IF NOT "%RIDER_PROPERTIES%" == "" SET IDE_PROPERTIES_PROPERTY="-Didea.properties.file=%RIDER_PROPERTIES%"

SET VM_OPTIONS_FILE=
SET USER_VM_OPTIONS_FILE=
IF NOT "%RIDER_VM_OPTIONS%" == "" (
  :: 1. %<IDE_NAME>_VM_OPTIONS%
  IF EXIST "%RIDER_VM_OPTIONS%" SET "VM_OPTIONS_FILE=%RIDER_VM_OPTIONS%"
)
IF "%VM_OPTIONS_FILE%" == "" (
  :: 2. <IDE_HOME>\bin\[win\]<exe_name>.vmoptions ...
  IF EXIST "%IDE_BIN_DIR%\rider64.exe.vmoptions" (
    SET "VM_OPTIONS_FILE=%IDE_BIN_DIR%\rider64.exe.vmoptions"
  ) ELSE IF EXIST "%IDE_BIN_DIR%\win\rider64.exe.vmoptions" (
    SET "VM_OPTIONS_FILE=%IDE_BIN_DIR%\win\rider64.exe.vmoptions"
  )
  ...
  ) ELSE IF EXIST "%APPDATA%\JetBrains\Rider2023.2\rider64.exe.vmoptions" (
    SET "USER_VM_OPTIONS_FILE=%APPDATA%\JetBrains\Rider2023.2\rider64.exe.vmoptions"
  )
)
